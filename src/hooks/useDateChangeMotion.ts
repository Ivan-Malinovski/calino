...
export function useDateChangeMotion(marker: string): DateChangeMotion {
  ...
  return useMemo(() => {
    ...
    if (prefersReducedMotion) {
      return { initial: false, animate: { opacity: 1, x: 0, y: 0 }, exit, transition: { duration: 0 } }
    }
    if (isMobile) {
    ...
    return {
      initial: { opacity: 0, y: forward ? -10 : 10 },
      animate: { opacity: 1, y: 0 },
      exit,
      transition: { duration: 0.1 },
    }
  }, [direction, isMobile, prefersReducedMotion])
}
