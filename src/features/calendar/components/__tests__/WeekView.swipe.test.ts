...
describe('shouldPageOnSwipe', () => {
  it('does not page while the day strip has room to scroll', () => {
    ...
    expect(shouldPageOnSwipe('left', scrolled(0))).toBe(false)
    expect(shouldPageOnSwipe('left', scrolled(maxScroll / 2))).toBe(false)
    expect(shouldPageOnSwipe('right', scrolled(maxScroll))).toBe(false)
    expect(shouldPageOnSwipe('right', scrolled(maxScroll / 2))).toBe(false)
  })

  it('pages once the strip is at the edge the swipe is heading towards', () => {
    expect(shouldPageOnSwipe('left', scrolled(maxScroll))).toBe(true)
    expect(shouldPageOnSwipe('right', scrolled(0))).toBe(true)
  })
  ...
  it('treats a fractional scroll position as being at the edge', () => {
    ...
    expect(shouldPageOnSwipe('left', scrolled(maxScroll - 0.6))).toBe(true)
    expect(shouldPageOnSwipe('right', scrolled(0.4))).toBe(true)
  })
  ...
  it('pages immediately when there is nothing to scroll', () => {
    ...
    expect(shouldPageOnSwipe('left', fits)).toBe(true)
    expect(shouldPageOnSwipe('right', fits)).toBe(true)
  })

  it('pages when there is no scroller at all', () => {
    expect(shouldPageOnSwipe('left', null)).toBe(true)
    expect(shouldPageOnSwipe('right', null)).toBe(true)
  })
})
