...
import { DEFAULT_BASE_URLS, type AIProvider, type ModelInfo, type TestConnectionResult } from '@/features/aiVision/types'
...
function RefreshIcon(): JSX.Element {
  return (
    <svg viewBox="0 0 18 18" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export function AIVisionSettings(): JSX.Element {
  ...
  const handleTest = async (): Promise<void> => {
    ...
    const result: TestConnectionResult = await testConnection({ provider, baseUrl, apiKey: key, model })
  ...
  return (
    <section className={`${styles.section} ${styles.sectionActive}`} data-component="ai-vision-settings">
      <h1 className={styles.pageTitle}>AI Photo Import</h1>
      <p className={styles.rowDesc} style={{ padding: '0 20px', marginBottom: 16 }}>
        Use your own API key to auto-fill event details from a photo. Available when creating a new event on Android.
      </p>
      ...
      <div className={styles.group}>
        ...
        <div className={styles.row} data-component="setting-row" data-setting="ai-vision-provider" data-value={provider}>
        ...
        <div className={styles.row} data-component="setting-row" data-setting="ai-vision-base-url">
          <div className={styles.rowInfo}>
          ...
          <div className={styles.rowControl}>
            {provider === 'custom' ? (
              ...
              <span className={styles.applySectionText} style={{ marginLeft: 0 }}>{resolvedBaseUrl}</span>
            )}
          </div>
        </div>

        <div className={styles.row} data-component="setting-row" data-setting="ai-vision-api-key">
          <div className={styles.rowInfo}>
        ...
        {keySaved && apiKeyInput === '' && (
          <div className={styles.accountStatus} style={{ padding: '0 20px 12px', marginTop: -6 }}>
            <div className={`${styles.statusDot} ${styles.statusDotOk}`} />
            Key saved
            <button
              type="button"
      ...
      <div className={styles.group}>
        <div className={styles.row} data-component="setting-row" data-setting="ai-vision-test">
          <div className={styles.rowInfo}>
            ...
            <div className={styles.rowDesc}>Verify the key works and the model can read images</div>
