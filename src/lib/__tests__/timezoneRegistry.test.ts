...
import { normalizeTzid, ensureZoneRegistered, resolveZone } from '../timezoneRegistry'
...
describe('timezoneRegistry', () => {
  ...
  describe('ensureZoneRegistered / resolveZone', () => {
    it('registers an IANA zone whose Timezone carries a VTIMEZONE component', () => {
    ...
    it('the registered zone knows DST: wall 10:00 is 09:00Z in March and 08:00Z in June', () => {
      const zone = resolveZone('Europe/Copenhagen')
      expect(zone).toBeDefined()
      const march = ICAL.Time.fromData({
        year: 2026, month: 3, day: 25, hour: 10, minute: 0, second: 0, zone: zone as never,
      })
      const june = ICAL.Time.fromData({
        year: 2026, month: 6, day: 1, hour: 10, minute: 0, second: 0, zone: zone as never,
      })
    ...
    it('resolves Windows IDs to a registered zone', () => {
